...
export async function setProductImages(productId: number, urls: string[]) {
  const before = await db.productImage.findMany({ where: { productId }, select: { url: true } });
  await db.$transaction([
    db.productImage.deleteMany({ where: { productId } }),
    db.productImage.createMany({ data: urls.map((url, position) => ({ productId, url, position })) }),
